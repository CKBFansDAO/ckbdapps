            ...
            <div className='flex flex-col items-center'>
                {
                    isLoading ? (<div className='h-[48px] w-14 flex items-center animate-pulse'>
                        ...
                        : (<div className='flex w-full items-end justify-center'>
                            <span className='text-[30px] text-[#232325] font-bold rounded-full px-2 py-1'>{epoch.number}</span>
                            <span className='text-sm text-[#232325] font-bold rounded-full px-1 py-1 mb-1'>{epoch.index}/{epoch.length}</span>
                        </div>
                        )
                }
    ...
    return (
        <div className='grid grid-cols-2 md:grid-cols-4 divide-x divide-y-reverse border-dashed py-10'>
            <SummaryItem value={blockNumber?.toLocaleString()}
                valueClass='text-[#232325] font-bold rounded-full px-5 py-1'
